...
- **OS**: Alma Linux 10, arm64 — running as a virtual machine inside macOS OrbStack
- **macOS mount points** (`/Users`, `/Volumes`) are exposed inside the VM; use them to access host files
- `/home` is the VM home directory and is **not** the macOS host home directory
- **Nix** is available with flakes support for tools not in `PATH`:
  ```
  nix run nixpkgs#ffmpeg -- --help
  ```
- **Docker** is available, including `docker buildx`:
  ```
  docker run ...
  docker buildx ...
  ```
...
- **Available**: bash, fish, nushell
- **Default login shell**: fish
- **Preferred scripting shell**: nushell — write shell scripts in nushell unless otherwise specified
- **Nushell note**: `get -i` is deprecated; use `get -o` / `get --optional` instead

## Git

- Git identity and commit signing are globally configured and auto-detected based on the working path
- No manual configuration needed; signing keys and identities are handled transparently
